...
#pragma clang diagnostic pop
    ...
    CGFloat totalHeight = imageHeight + spacing + labelHeight;
    CGFloat verticalInset = (CGRectGetHeight(self.bounds) - totalHeight) / 2.0;
    CGFloat totalWidth = imageWidth + spacing + labelWidth;
    CGFloat contentInset = (CGRectGetWidth(self.bounds) - totalWidth) / 2.0;
    spacing = spacing / 2;
    switch (postitionStatus) {
        ...
        case WYJImagePositionRight: {
            ...
            self.imageEdgeInsets = UIEdgeInsetsMake(0, labelWidth + inset, 0, -labelWidth - inset);
            self.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
        } break;
            
        case WYJImagePositionTop: {
            self.imageEdgeInsets = UIEdgeInsetsMake(-labelHeight - spacing + verticalInset, (labelWidth / 2.0), 0, -(labelWidth / 2.0));
            self.titleEdgeInsets = UIEdgeInsetsMake(imageHeight + spacing + verticalInset, -(imageWidth / 2.0), 0, imageWidth / 2.0);
            self.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
        } break;
...
- (void)yi_setGradientTextColor:(NSString *)hexColorString {
    ...
    if (hexColorString.length == 0 || hexColorString == nil) {
        self.yi_titleColor = UIColor.whiteColor;
        return;
    }
