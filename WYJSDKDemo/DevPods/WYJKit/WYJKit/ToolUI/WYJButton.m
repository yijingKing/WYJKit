...
- (void)initialize {
    _imagePosition = WYJButtonImagePositionLeft;
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case WYJButtonImagePositionLeft: {
...
- (void)sizeToFit {
    ...
    switch (self.imagePosition) {
        case WYJButtonImagePositionLeft:
        case WYJButtonImagePositionRight: // 图片在左右时，在系统计算好的基础上宽度再加间距
            myFrame.size.width = self.frame.size.width + _imageTitleSpace;
            break;
        case WYJButtonImagePositionTop:
        case WYJButtonImagePositionBottom: {// 图片在上下时，就不能再在系统计算的基础上增减值了，因为系统计算是基于图片在左文字在右时进行的，宽度依赖图片+文字之和，而图片在上下时，宽度应该依赖图片和文字较大的那个
...
- (void)setImagePosition:(WYJButtonImagePosition)imagePosition {
